/**
 * Implements native harness hook event relays behind the public SDK facade.
 */
...
export type NativeHookRelayRetention = Readonly<{
  ...
  allowPreToolUse: (rawPayload: Parameters<typeof readCodexNativeAgentId>[0]) => boolean;
  awaitForegroundAdmission?: (
    rawPayload: Parameters<typeof readCodexNativeAgentId>[0],
...
export function registerNativeHookRelayForPublicFacade(
...
function registerNativeHookRelayInternal(
  ...
  try {
    ...
    registerNativeHookRelayBridge(registration, stateDbPath, invokeNativeHookRelayFromRegistry);
...
async function resolveNativeHookRelayInvocationBinding(
  ...
  const nativeAgentId = readCodexNativeAgentId(rawPayload);
  if (nativeAgentId && event === "pre_tool_use") {
    ...
    if (lifetime.foregroundOpen && lifetime.retention?.awaitForegroundAdmission) {
      assertAdmission = await lifetime.retention.awaitForegroundAdmission(rawPayload);
      if (
        !assertAdmission ||
        relays.get(registration.relayId) !== registration ||
        Date.now() > registration.expiresAtMs ||
        !lifetime.retention.allowPreToolUse(rawPayload)
      ) {
        throw new Error("native hook relay retained invocation not allowed");
      }
      registration.signal?.throwIfAborted();
      retained.assertActive();
    } else if (lifetime.retention?.allowPreToolUse(rawPayload) !== true) {
      throw new Error("native hook relay retained invocation not allowed");
    }
    const assertActive = () => {
      if (
      ...
      if (lifetime.retention?.allowPreToolUse(rawPayload) !== true) {
        throw new Error("native hook relay retained invocation not allowed");
      }
    };
    return {
...
export async function invokeNativeHookRelayFromRegistry(
...
export function hasNativeHookRelayInvocationFromRegistry(params: {
  relayId: string;
  event: NativeHookRelayEvent;
  toolUseId?: string;
}): boolean {
...
export const nativeHookRelayTestingImplementation = {
